...
const TutorLogin = () => {
    ...
    const renderCalendarCells = () => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            cells.push(<div key={`day-${day}`} className={`p-2 border border-gray-200 dark:border-gray-700 min-h-[80px] ${hasLesson ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                ...
                {hasLesson && <div className="mt-1 space-y-1">
                    {lessonDates[day].map((lesson, idx) => <div key={idx} className="text-xs p-1 bg-blue-100 dark:bg-blue-800 rounded mb-1 truncate">
                        ...
                        <div className="text-xs text-gray-700 dark:text-gray-300 truncate">{lesson.studentName}</div>
                        <div className="text-xs text-gray-700 dark:text-gray-300 truncate">{lesson.subject}</div>
                    </div>)}
                </div>}
            </div>);
        }
    ...
    return <main className="w-full bg-white dark:bg-gray-900 min-h-screen">
        {!isLoggedIn ? <section className="py-16 md:py-24 container mx-auto px-4">
            ...
            {activeTab === 'messages' && <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                <h2 className="text-xl font-semibold mb-6 dark:text-white">
                    Messages
                </h2>
                <div className="flex flex-col md:flex-row h-[70vh]">
                    ...
                    <div className={`md:w-1/3 flex-shrink-0 border-r border-gray-200 dark:border-gray-700 overflow-y-auto ${selectedChat ? 'hidden md:block' : 'block'}`}>
                        {messages.map(chat => <div key={chat.parentId} onClick={() => setSelectedChat(chat)} className={`flex items-center p-4 border-b border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 ${selectedChat?.parentId === chat.parentId ? 'bg-gray-50 dark:bg-gray-700' : ''}`}>
                    ...
                    <div className={`flex-1 flex flex-col ${selectedChat ? 'block' : 'hidden md:block'}`}>
                        {selectedChat ? <>
                            <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center">
                                <button onClick={() => setSelectedChat(null)} className="md:hidden mr-2 text-gray-500 dark:text-gray-400">
                                    <ChevronLeftIcon className="h-6 w-6" />
                                </button>
                                <img className="h-10 w-10 rounded-full object-cover mr-3" src={selectedChat.parentImage} alt={selectedChat.parentName} />
                                <h3 className="text-lg font-medium text-gray-900 dark:text-white">
                                    {selectedChat.parentName}
                                </h3>
                            </div>
                            <div className="flex-1 p-4 overflow-y-auto space-y-4">
                                {selectedChat.messages.map((message, index) => <div key={index} className={`flex ${message.sender === 'tutor' ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-xs md:max-w-md px-4 py-2 rounded-lg ${message.sender === 'tutor' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white'}`}>
                                        <p>{message.text}</p>
                                        <span className={`block text-right text-xs mt-1 ${message.sender === 'tutor' ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400'}`}>
                                            {message.timestamp}
                                        </span>
                                    </div>
                                </div>)}
                            ...
                            <div className="p-4 border-t border-gray-200 dark:border-gray-700">
                                <form className="flex">
                                    <input type="text" placeholder="Type a message..." className="flex-1 px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-l-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-white" />
            ...
            {activeTab === 'progress' && <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {studentProgress.map(student => <div key={student.id} className="p-6 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm">
                        ...
                        <div className="mt-4">
                            ...
                            <div className="mt-1 flex flex-wrap gap-2">
                                {student.grades.map((grade, index) => <span key={index} className="inline-block px-2 py-1 text-xs font-semibold bg-gray-200 dark:bg-gray-600 rounded-full text-gray-800 dark:text-gray-100">
                                    {student.tests[index]}: {grade}%
                                </span>)}
                            </div>
                        </div>
                        <div className="mt-4">
                ...
                {showProgressInput && <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-2xl">
                        ...
                        <form onSubmit={handleNewProgressSubmit}>
                            <div className="mb-4">
                                <label htmlFor="newGrade" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                    New Test/Assignment Name and Grade
                                </label>
                                <div className="mt-1 flex gap-2">
                                    <input type="text" id="newTest" placeholder="e.g. 'Final Exam'" className="flex-1 px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md dark:text-white" required />
                                    <input type="number" id="newGrade" placeholder="e.g. 95" className="w-24 px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md dark:text-white" required />
                                </div>
                            </div>
                            <div className="mb-4">
                                <label htmlFor="newNotes" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                    Progress Notes
                                </label>
                                <textarea id="newNotes" rows={3} placeholder="Add new notes about their progress..." className="mt-1 w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md dark:text-white"></textarea>
                            </div>
                            <div className="flex justify-end gap-2">
                                <button type="button" onClick={() => setShowProgressInput(false)} className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-700 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600">
                                    Cancel
                                </button>
